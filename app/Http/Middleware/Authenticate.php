...
class Authenticate
{
    ...
    private function handleRedirect($request, $utilityController, $aesController, $cond, $message = 'Unauthorized', $statusCode, $link = '/login'){
        if($cond == 'error'){
            setcookie('token1', '', ['expires'  => time() - 3600, ...self::$metaDelCookie]);
            setcookie('token2', '', ['expires'  => time() - 3600, ...self::$metaDelCookie]);
            return !$request->isMethod('get') || $request->wantsJson() ? $utilityController->getView($request, $aesController, '', ['message'=>$message], 'json_encrypt', $statusCode) : redirect('/login');
        }
        return !$request->isMethod('get') || $request->wantsJson() ? $utilityController->getView($request, $aesController, '', ['message'=>'redirect'], 'json_encrypt', 302) : redirect($link);
    }
    public function handle(Request $request, Closure $next){
        ...
        if(isset($_COOKIE['token2'])){
            ...
            if(!isset($token2['value']) && !isset($token2['exp'])){
                return $this->handleRedirect($request, $utilityController, $aesController, 'error', 'Invalid Token', 500);
            }
            ...
            if($isPath($currentPath, $publicPage, $prefPublic) || $isPath($currentPath, array_map(fn($path) => '/api' . $path, $publicPage), array_map(fn($path) => '/api' . $path, $prefPublic))){
                if(in_array(ltrim($path), $publicPage)){
                    $response = $this->handleRedirect($request, $utilityController, $aesController, 'success', '/dashboard');
                }else{
                    ...
                    if(is_null($response) && $request->isMethod('get') && !in_array($path, ['/download'])){
                        $response = $this->handleRedirect($request, $utilityController, $aesController, 'success', $path ?? '/dashboard');
                    }
                }
                return $response;
            }
            ...
            if(is_null($userDb)){
                return $this->handleRedirect($request, $utilityController, $aesController, 'error', 'User Not Found', 404);
            }
            ...
            $upToken1 = function() use ($request, $jwtController, $utilityController, $aesController, $decodedRefresh, $userDb, $next){
                ...
                if($updated['status'] == 'error'){
                    return $this->handleRedirect($request, $utilityController, $aesController, 'error', 'Update token error', 500);
                }
            ...
            if($decoded['status'] == 'error'){
                ...
                return $this->handleRedirect($request, $utilityController, $aesController, 'error', $decoded['message'], 500);
            }
